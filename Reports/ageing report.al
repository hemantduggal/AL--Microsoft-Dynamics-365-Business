/// <summary>
/// Ageing Report (ID 50108).
/// </summary>
report 50108 "Ageing Report"
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    Caption = '30 days receivable ageing report ';
    //DefaultLayout = RDLC;
    // RDLCLayout = 'ReportLayout\30DReceivableAging.rdl';//phle tha 'ReportLayout\Ageing report.rdl'

    //for excel buffer enabe this 
    ProcessingOnly = true;
    ...
    dataset
    {
        dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
        {
            ...
            begin
                ...
                // "Cust. Ledger Entry".SetRange("Posting Date", PeriodStartDate.Get(Integer.Number), PeriodEndDate.Get(Integer.Number));
                // IF "Cust. Ledger Entry".FindSet() then
                //     repeat
                //         "Cust. Ledger Entry".CalcFields("Original Amount");
                //         TotalAmount := TotalAmount + "Cust. Ledger Entry"."Original Amount";
                //     until ("Cust. Ledger Entry".Next() = 0);
                //* EXTRA ONLYYYYY no work 

                ///for excel buffer report--->
                /// 
                "Cust. Ledger Entry".Reset();
            ...
            /// trigger excel buffer ka       
            trigger OnPostDataItem()
            begin
                ExcelBuffer.NewRow;
                ExcelBuffer.NewRow();
                ExcelBuffer.AddColumn('Totals', false, '', true, false, true, '', ExcelBuffer."Cell Type"::Text);
                ...
                ExcelBuffer.AddColumn(TOtalRemainAMt, FALSE, '', true, FALSE, FALSE, '', ExcelBuffer."Cell Type"::Number);
    ...
    begin
        ExcelBuffer.NewRow();
        ExcelBuffer.AddColumn("Cust. Ledger Entry"."Customer No.", false, '', true, false, true, '', ExcelBuffer."Cell Type"::Number);
